...
This file documents every issue encountered while building the SOC layer (see [08-soc-configuration.md](../08-soc-configuration.md)). Each entry follows the same structure: symptom → diagnosis → root cause → fix. The `TS-x` identifiers are referenced from the main procedure.
...
**Fix.** Local Postfix relay on loopback: maild speaks plain SMTP to `127.0.0.1`, Postfix relays to `smtp.gmail.com:587` with SASL auth (Gmail App Password) and TLS. Full setup in [08-soc-configuration.md](../08-soc-configuration.md) section 5. Delivery confirmed end-to-end.
